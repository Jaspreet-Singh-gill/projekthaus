...
import { Skeleton, Separator } from "../ui/index.js";
...
const SideBar = ({ className }) => {
    ...
    // Consolidated link styling helper
    const getLinkClass = (isActive, isProject = false) => {
        const base = "flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200";
        if (isActive) {
            return `${base} bg-violet-500/10 dark:bg-violet-600/10 text-violet-600 dark:text-violet-400 font-semibold ${
                isProject ? "border-l-2 border-violet-500 pl-2.5" : ""
            }`;
        }
        return `${base} text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-900/60`;
    };

    const projects = projectsList.data?.data || [];
                            ...
                            projects.map((project) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        className={getLinkClass(isActive, true)}
                                    >
