...
const TaskListTable = ({ taskData, columns }) => {
    ...
    return (
        <div className="overflow-x-auto overflow-y-auto rounded-2xl border border-slate-200 dark:border-slate-900 bg-white dark:bg-slate-950/20 backdrop-blur-md shadow-lg transition-colors duration-200">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-b border-slate-200 dark:border-slate-900 bg-slate-50 dark:bg-slate-950/40">
                ...
                <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto justify-end">
                    <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-1.5 hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                        <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Priority</span>
                        <div className="relative flex items-center">
                            <select
                                className="bg-transparent text-sm text-slate-800 dark:text-slate-200 focus:outline-none cursor-pointer appearance-none pr-5 font-medium"
                    ...
                    <button
                        className={`flex items-center gap-2 px-4 py-1.5 rounded-xl text-sm font-semibold transition-all duration-200 border cursor-pointer ${assignedToMe
                                ? "bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/30 shadow-[0_0_12px_rgba(99,102,241,0.15)]"
                                : "bg-slate-100 dark:bg-slate-900/30 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:text-slate-900 dark:hover:text-slate-200 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-900/50"
                            }`}
                        onClick={() => {
            ...
            <table className="w-full border-collapse text-left text-sm overflow-x-scroll">
                <thead className="bg-slate-100 dark:bg-slate-900/60 border-b border-slate-200 dark:border-slate-900">
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-900/50 bg-white dark:bg-slate-950/5">
                    {table.getRowModel().rows.map((row) => (
                        ...
                        >
                            {row.getVisibleCells().map((cell) => (
                                <td
                                    key={cell.id}
                                    className="px-6 py-4 text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap"
                                >
                                    {flexRender(
