...
const updateSubTask = asyncHandler(async (req, res, next) => {
  ...
  if (!subTaskId || !taskId) {
    throw new ApiError(400, "", "SubtasId is required to update the task");
  }
  try {
    if (!name) {
      throw new ApiError(400, "", "Name of the task is required");
...
const deleteSubTask = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    await Comment.deleteMany({ subtaskId: subTaskId });
    await Notification.deleteMany({ subtaskId: subTaskId });
...
const getAllTheSubTask = asyncHandler(async (req, res, next) => {
  const { taskId } = req.params;
  if (!taskId) {
    throw new ApiError(400, "", "taskId is required to update the task");
  }
